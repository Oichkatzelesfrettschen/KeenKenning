/*
 * KenningStoryManager.kt: Story Mode chapter and progress management (Phase 4c)
 *
 ...
 */
class KenningStoryManager(private val context: Context) : StoryManager {

    companion object {
        private const val PREFS_NAME = "keenkenning_story_progress"
        ...
        private val BOOK_IV_CHAPTERS = listOf(
            ...
            StoryChapter(
                ...
                outroNarrative = """
                    ...
                    |Thank you for playing Keen Kenning.
                """.trimMargin(),
                bookId = "book4"
            )
        )
    ...
    override fun getProgress(): StoryProgress {
    ...
    override fun getChapter(chapterId: String): StoryChapter? =
        CHAPTERS.find { it.id == chapterId }

    override fun setCurrentChapter(chapterId: String?) {
        prefs.edit().putString(KEY_CURRENT_CHAPTER, chapterId).apply()
    }

    override fun recordPuzzleCompleted(chapterId: String) {
    ...
    override fun getAllChapters(): List<StoryChapter> = CHAPTERS

    override fun getAllBooks(): List<StoryBook> = BOOKS

    override fun getBook(bookId: String): StoryBook? = BOOKS.find { it.id == bookId }

    override fun getCurrentBook(): StoryBook? {
        val bookId = prefs.getString(KEY_CURRENT_BOOK, null)
        return bookId?.let { getBook(it) } ?: BOOKS.firstOrNull()
    }

    override fun setCurrentBook(bookId: String) {
        prefs.edit().putString(KEY_CURRENT_BOOK, bookId).apply()
    }

    override fun isBookUnlocked(bookId: String): Boolean {
    ...
    override fun getBookProgress(bookId: String): Float {
    ...
    override fun getTotalProgress(): Float {
    ...
    private val narrativeGenerator: NarrativeGenerator by lazy {
        KenningNarrativeGenerator.getInstance(context)
    }
     ...
     */
    override fun releaseNarrativeGenerator() {
