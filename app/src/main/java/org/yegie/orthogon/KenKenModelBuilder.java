/*
 * KenKenModelBuilder.java: JNI bridge for puzzle generation
 *
 ...
 * or AI-assisted generation) and parsing it into a KenKenModel.
 *
 ...
 */
public class KenKenModelBuilder {
    ...
    //of the library in C and then creates a KenKenModel based off of it.
    public KenKenModel build(android.content.Context context, int size, int diff, int multOnlt, long seed, boolean useAI) {
     ...
     */
    public KenKenModel build(android.content.Context context, int size, int diff, int multOnlt, long seed, boolean useAI, int modeFlags)
    {
        String levelAsString = null;
        boolean aiActuallyUsed = false;
        NeuralKenKenGenerator.GenResult aiResult = null;

        if (useAI) {
            NeuralKenKenGenerator aiGen = new NeuralKenKenGenerator();
        ...
        KenKenModel.GridCell[][] cells = new KenKenModel.GridCell[size][size];
        ...
        KenKenModel.Zone[] zones = new KenKenModel.Zone[zoneCount];
        ...
        for(int i = 0; i<zoneCount; i++)
        {
            ...
            {
                case 'a':
                    zones[i] = new KenKenModel.Zone(KenKenModel.Zone.Type.ADD,val,i);
                    break;
                case 'm':
                    zones[i] = new KenKenModel.Zone(KenKenModel.Zone.Type.TIMES,val,i);
                    break;
                case 's':
                    zones[i] = new KenKenModel.Zone(KenKenModel.Zone.Type.MINUS,val,i);
                    break;
                case 'e':
                    zones[i] = new KenKenModel.Zone(KenKenModel.Zone.Type.EXPONENT,val,i);
                    break;
                default://divide
                    zones[i] = new KenKenModel.Zone(KenKenModel.Zone.Type.DIVIDE,val,i);
                    break;
            }
        }
        ...
        for(int i = 0; i<size*size; ++i)
        {
            ...
            cells[x][y] = new KenKenModel.GridCell(val,zones[zoneIndex]);
        ...
        return new KenKenModel(size,zones,cells, aiActuallyUsed);
