 ...
 *
 * This abstraction decouples KenKenModel from rendering concerns,
 ...
 * without depending on KenKenModel internals.
 */
 ...
 * Maps to KenKenModel.Zone.Type but decoupled from it.
 */
...
/**
 * Transforms KenKenModel into RenderableGameState.
 *
...
object GameStateTransformer {
    ...
    /**
     * Transform a KenKenModel into a RenderableGameState.
     *
    ...
    private fun transformZones(model: KenKenModel, gameMode: GameMode): List<RenderableZone> {
    ...
    private fun mapOperation(type: KenKenModel.Zone.Type): ZoneOperation {
        return when (type) {
            KenKenModel.Zone.Type.ADD -> ZoneOperation.ADD
            KenKenModel.Zone.Type.MINUS -> ZoneOperation.SUBTRACT
            KenKenModel.Zone.Type.TIMES -> ZoneOperation.MULTIPLY
            KenKenModel.Zone.Type.DIVIDE -> ZoneOperation.DIVIDE
            KenKenModel.Zone.Type.EXPONENT -> ZoneOperation.EXPONENT
        }
    }

    private fun formatClueLabel(
        zone: KenKenModel.Zone,
        gameMode: GameMode,
    ...
    private fun transformCells(model: KenKenModel, size: Int): List<List<RenderableCell>> {
        val grid = model.grid ?: return emptyList()
    ...
    private fun calculateBorders(
        grid: Array<Array<KenKenModel.GridCell?>>,
        x: Int,
        y: Int,
        size: Int
    ): CageBorders {
