...
public class NeuralKeenGenerator {
    ...
    public GenResult generate(Context context, int size) {
        ...
        // 1. Try ONNX (Supports sizes 3-16 with latin_solver.onnx)
    ...
    private GenResult generateFromModel(Context context, int size) throws Exception {
        ...
        java.io.File modelFile = new java.io.File(cacheDir, "latin_solver.onnx");
        java.io.File dataFile = new java.io.File(cacheDir, "latin_solver.onnx.data");

        copyAsset(context, "latin_solver.onnx", modelFile);
        // Data file is optional (only if model uses external data storage)
        try {
            copyAsset(context, "latin_solver.onnx.data", dataFile);
        } catch (IOException e) {
