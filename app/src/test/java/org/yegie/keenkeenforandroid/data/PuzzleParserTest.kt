...
class PuzzleParserTest {
    ...
    fun `parse valid 3x3 puzzle`() {
        ...
        assertEquals(KeenModel.Zone.Type.ADD, puzzle.zones[0].operation)
        assertEquals(6, puzzle.zones[0].targetValue)
        assertEquals(KeenModel.Zone.Type.TIMES, puzzle.zones[1].operation)
    ...
    fun `parse valid 4x4 puzzle with all operations`() {
        ...
        assertTrue(KeenModel.Zone.Type.ADD in ops)
        assertTrue(KeenModel.Zone.Type.TIMES in ops)
        assertTrue(KeenModel.Zone.Type.MINUS in ops)
        assertTrue(KeenModel.Zone.Type.DIVIDE in ops)
        assertTrue(KeenModel.Zone.Type.EXPONENT in ops)
    }
    ...
    fun `isValidLatinSquare validates correctly`() {
        ...
        val validPuzzle = ParsedPuzzle(
            size = 3,
            zones = listOf(ParsedZone(KeenModel.Zone.Type.ADD, 6, 0)),
            cells = listOf(
