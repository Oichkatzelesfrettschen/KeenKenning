...
class JniResultParserTest {
    ...
    @Test
    fun `toGenerationFailure maps INVALID_GRID to MlGenerationFailed`() {
        val jniResult = JniResult.Error(JniErrorCode.INVALID_GRID, "AI grid rejected")
        val failure = JniResultParser.toGenerationFailure(jniResult)

        assertTrue(failure is PuzzleGenerationResult.Failure.MlGenerationFailed)
        val mlFailure = failure as PuzzleGenerationResult.Failure.MlGenerationFailed
        assertFalse(mlFailure.fallbackAttempted)
    }
