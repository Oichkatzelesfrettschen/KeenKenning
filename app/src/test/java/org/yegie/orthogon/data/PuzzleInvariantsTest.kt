...
class PuzzleInvariantsTest {
    ...
    fun `zone constraint - addition zones sum to target`() {
        val puzzle = generatePuzzleWithZoneTypes(
            size = 4,
            operations = listOf(KenKenModel.Zone.Type.ADD),
            seed = 3000L
        )
    ...
    fun `zone constraint - multiplication zones multiply to target`() {
        val puzzle = generatePuzzleWithZoneTypes(
            size = 4,
            operations = listOf(KenKenModel.Zone.Type.TIMES),
            seed = 4000L
        )
        for ((zoneIndex, zone) in puzzle.zones.withIndex()) {
            val zoneCells = puzzle.cells.filter { it.zoneIndex == zoneIndex }
    ...
    fun `zone constraint - subtraction zones yield target (larger minus smaller)`() {
        val puzzle = generatePuzzleWithZoneTypes(
            size = 4,
            operations = listOf(KenKenModel.Zone.Type.MINUS),
            seed = 5000L,
            maxZoneSize = 2 // Subtraction only valid for 2-cell zones
        )
        for ((zoneIndex, zone) in puzzle.zones.withIndex()) {
            if (zone.operation != KenKenModel.Zone.Type.MINUS) continue
    ...
    fun `zone constraint - division zones yield target (larger divided by smaller)`() {
        val puzzle = generatePuzzleWithZoneTypes(
            size = 4,
            operations = listOf(KenKenModel.Zone.Type.DIVIDE),
            seed = 6000L,
            maxZoneSize = 2 // Division only valid for 2-cell zones
        )
        for ((zoneIndex, zone) in puzzle.zones.withIndex()) {
            if (zone.operation != KenKenModel.Zone.Type.DIVIDE) continue
    ...
    private fun generateValidLatinSquare(size: Int, seed: Long): ParsedPuzzle {
        ...
        val zones = listOf(ParsedZone(KenKenModel.Zone.Type.ADD, totalSum, 0))
    ...
    private fun generatePuzzleWithZoneTypes(
        size: Int,
        operations: List<KenKenModel.Zone.Type>,
        seed: Long,
     ...
     */
    private fun calculateZoneTarget(digits: List<Int>, operation: KenKenModel.Zone.Type): Int {
        return when (operation) {
            KenKenModel.Zone.Type.ADD -> digits.sum()
            KenKenModel.Zone.Type.TIMES -> digits.fold(1) { acc, d -> acc * d }
            KenKenModel.Zone.Type.MINUS -> {
                if (digits.size == 2) kotlin.math.abs(digits[0] - digits[1])
                else digits.sum() // Fallback for >2 cells
            }
            KenKenModel.Zone.Type.DIVIDE -> {
                if (digits.size == 2) {
                    val (a, b) = digits.sortedDescending()
                    if (b != 0 && a % b == 0) a / b else 1
                } else 1 // Fallback
            }
            KenKenModel.Zone.Type.EXPONENT -> {
                if (digits.size == 2) {
                    val (base, exp) = digits
